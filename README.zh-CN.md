...
DeepSeek Harness (DSH) 的**已归档对话管理器**插件。它在 **设置 → 已归档** 中按项目分组列出所有已归档对话,并支持**取消归档**与**删除**。
...
- 设置页每 3 秒自动刷新,与 DSH 的 `host/archived-sessions-changed` 广播保持一致。
...
- 安全护栏:活跃(已打开)的会话不允许取消归档 / 删除,避免破坏正在进行的会话。
