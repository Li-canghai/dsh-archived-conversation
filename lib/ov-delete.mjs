...
export function pendingStore(file = prepareRuntimeFile(OV_PENDING_FILE)) {
  return {
    file,
    async load() {
      try {
    ...
    async save(entries) {
      ...
      atomicWriteFileSync(file, JSON.stringify(entries, null, 2), { encoding: "utf8", mode: 0o600 });
    },
  };
}
...
let ovQueueTail = Promise.resolve();

export function withOvQueue(fn) {
  const run = ovQueueTail.then(fn, fn);
  ovQueueTail = run.then(() => undefined, () => undefined);
  return run;
}

export function ovQueueSettled() {
  return ovQueueTail;
}

export async function pushOvPendingDelete({ sessionId, store = pendingStore(), logger = null }) {
  try {
    await withOvQueue(async () => {
