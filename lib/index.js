...
// OpenViking delete-linkage switch (see SPEC-openviking-delete-link.md §8.1):
// default on; set DSH_ARCHIVED_CONVERSATION_OV_LINK=0 or =false to disable.
const ovLinkEnabled = () => {
...
// Durable title read for the supported DSH alpha.3/alpha.4 contract. Use the
// zero-I/O cachedSnapshot(header) fast path, then inspect the complete logical
// log and fold its last `session/title` event. Unlike readFrom, inspect has no
// event-seq/log-offset argument whose type changed in alpha.4.
// `headerResolved` distinguishes a missing header from one the caller has not
// attempted to resolve yet.
async function readLogTitle(ctx, id, header, headerResolved = false) {
  try {
    ...
    if (cache !== undefined && typeof cache.cachedSnapshot === "function") {
      // Zero-read fast path: the persisted cache row (archived sessions were
      // active at some point, so they usually have one).
      try {
        const h = headerResolved ? header : await ctx.workspaceRegistry.readSessionHeader(id);
        if (h !== void 0) {
          // rc.1 signature: cachedSnapshot(meta, inheritedEventCount, keys).
          // An archived (cold) session is unseeded, so pass the zero cut
          // (0 → SessionLogOffset(0)) to match the stored record identity;
          // a seeded lifecycle simply misses and falls back to inspect().
          // keys=["title"] keeps the checkpoint row read minimal.
  ...
  try {
    const { events } = await ctx.sessionPersistence.inspect(id);
...
async function buildArchivedList(ctx) {
  ...
  const childrenByRoot = new Map(
    await Promise.all(archivedIds.map(async (id) => [id, await subagentChildrenOf(ctx, id)])),
  );
  ...
  const fileStats = new Map(
    await Promise.all(allSessionIds.map(async (id) => [id, await sessionFileStat(id)])),
  );
...
async function attemptDelete(ctx, id, options = {}) {
  ...
  // OpenViking linkage: delete the corresponding session record
  // (`dsh-<id>`) once the local session is gone. Best-effort — this never
  // blocks or fails the local deletion (skipped/queued/deleted semantics in
  // ov-delete.mjs; queued entries are replayed at boot / timer / page open).
  ...
  // A cold session has no Session instance whose lifecycle can emit
  // session/disposed. Notify only the API list with its documented id payload;
  // forging `{ id }` as a Session would invoke every host lifecycle listener
  // with an invalid object. Attached deletion already relays this event from
  // the real session/disposed notification during loop teardown.
  if (!wasAttached) {
    try {
...
export function apply(ctx) {
  ...
  // OpenViking pending-delete replay: same cadence as the deferred-delete queue
  // (boot + 20s timer); the settings page also triggers it on /list below.
  const ovSweep = () => {
    ...
    async handler(req, res) {
      ...
      try {
        ...
        if (req.method === "GET" && rest === "/list") {
          // Opening the archived page is also a good moment to replay any
          // pending OpenViking deletions (fire-and-forget, never blocks list).
          ovSweep();
        ...
        if (req.method === "POST" && unarchiveMatch) {
          ...
          return json(res, result.ok ? 200 : result.conflict ? 409 : 404, result);
        }
        ...
        if (req.method === "DELETE" && idMatch) {
          ...
          return json(res, result.conflict ? 409 : 404, result);
        }
