...
const SESSIONS_BASE = join(homedir(), ".dsh", "sessions");
...
// In-memory caches so the 3s client poll never re-reads session logs.
...
// cache (`coldSnapshot`) serves sessions that were active at some point with a
// tail-only read; otherwise fold the last `session/title` event from the log
// (the same logic as DSH's own "title" projection unit).
async function readLogTitle(ctx, id) {
  try {
    ...
    if (cache !== undefined) {
      ...
      // 2) Cache plus incremental tail read.
...
// Cached title read: reuses the previous result while the log file is unchanged.
async function titleFor(ctx, id) {
  ...
  const title = await readLogTitle(ctx, id);
  titleCache.set(id, { fp, title });
  return title;
}
...
async function listArchived(ctx) {
  ...
  const titleMap = {};
  const titleResults = await Promise.all(archivedIds.map(async (id) => [id, await titleFor(ctx, id)]));
  ...
  for (const id of archivedIds) {
    ...
    if (!cwd) {
      try {
        ...
        // header unavailable; fall back to the raw id
      }
    }
