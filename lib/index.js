...
// drop from the set, then remove the session dir; active (attached) sessions
// are refused so we never delete a conversation that is currently open.
...
// `session/title` event from the log. `header` lets callers that batch-resolved
// headers once (listArchived) avoid re-triggering a full list() per session.
...
async function listArchived(ctx) {
  ...
  // cold-miss fills the registry's header cache) so the zero-I/O cachedSnapshot
  // path can serve all titles without per-session re-triggering list().
...
// when gone, or { done: false, error } when DSH refuses to release it (the
// caller queues the id). Never mutates state before the session is released.
async function attemptDelete(ctx, id, options = {}) {
  ...
  if (wasAttached) {
    ...
    // Right after a turn the agent may still be settling back to idle; give a
    // short grace window. Background sweeps use quick mode and skip the wait.
