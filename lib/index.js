...
import { atomicWriteFileSync, prepareRuntimeFile } from "./runtime-paths.mjs";
...
async function releaseAttachedSession(ctx, id, { quick }) {
  const handle = agentHandles.get(id)?.deref();
...
async function purgeChangeLedgerCheckpoints(ctx, sessionId, cwd) {
  ...
  if (typeof ledger?.deleteBySession !== "function") return;
  try {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      pendingDeletes = new Set(
        parsed.filter((entry) => typeof entry === "string" && /^[A-Za-z0-9_-]+$/.test(entry)),
      );
    }
  } catch {
    pendingDeletes = new Set();
...
function cachedTitleFor(id, st) {
  ...
  const hit = persistedTitles.get(id);
...
async function unarchive(ctx, id) {
  ...
  for (const familyId of familyIds) {
    if (pendingDeletes.delete(familyId)) cancelled = true;
  }
  if (cancelled) savePendingDeletes(ctx.logger);
  return { ok: true };
}
...
async function attemptDelete(ctx, id, options = {}) {
  ...
  const { ov } = await finalizeDeletedSession(ctx, id, { wasAttached, cwd });
  return { done: true, ov };
}

async function finalizeDeletedSession(ctx, id, { wasAttached, cwd }) {
  ...
  for (const id of [...pendingDeletes]) {
    try {
      ...
      if (!state.archivedSessionIds.includes(id)) {
        ...
        if (isActive(ctx, id) || owningWorkspace(ctx, id) !== null) {
          pendingDeletes.delete(id);
          ctx.logger?.warn(`archived-conversation: queued delete for ${id} cancelled; the session is still live`);
          continue;
        }
        try {
        ...
        await finalizeDeletedSession(ctx, id, { wasAttached: false });
        continue;
      }
