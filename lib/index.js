...
// Durable title read. The cross-version projection-cache contract is only the
// zero-I/O cachedSnapshot(header) fast path. coldSnapshot changed from
// (id, signal?) to (header, events) in DSH 0.1.2-alpha.1, so a cache miss goes
// straight to one persistence read and folds the last `session/title` event.
