// Client half (browser) for dsh-archived-conversation.
//
// Registers a "已归档" tab under Settings via the `settings.section` slot, then
// renders archived conversations grouped by project with unarchive / delete
// actions. Plain JavaScript + react.createElement (no JSX, no bundler). Talks to
// the host only through the same-origin JSON API /archived-conversation/api/*.
//
// The wiring mirrors dsh-mcp-manager: a `window.__ModuleLoader__.load` factory
// whose `require("react")` comes from the DSH client bundle, and a plugin that
// injects the `slots` service to register the settings section.
    ...
    function ArchivedSection() {
      ...
      const refresh = react.useCallback(() => {
        api("/list")
          .then((r) => {
            ...
            // refresh fire constantly, and with many archived sessions a full
            // list rebuild on every tick is wasted work. JSON compare is fine
            // at this size and far cheaper than re-rendering every row.
      ...
      const unarchive = async (id) => {
        ...
        try {
          ...
          if (!r.ok) setError(r.body.error || `取消归档失败 (HTTP ${r.status})`);
          else refresh();
        } catch (e) {
          setError(String(e));
        }
      ...
      return createElement(
        ...
        createElement(
          ...
          createElement("span", null, String(total)),
        ),
