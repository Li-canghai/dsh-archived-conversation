window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function api(path, options) {
      return fetch("/api/archived-conversation" + path, {
    ...
    function ArchivedSection() {
      ...
      const unarchive = async (id) => {
        ...
        setError("");
        try {
          const r = await api("/unarchive", { method: "POST", body: JSON.stringify({ id }) });
      ...
      const del = async (id) => {
        ...
        setConfirmId(null);
        try {
          const r = await api("/delete", { method: "POST", body: JSON.stringify({ id }) });
